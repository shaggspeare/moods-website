...
const Home2Courses: React.FC = () => {
  // Type assertion to ensure Content has the expected structure
  const typedContent = Content as CoursesContent;
      ...
      <div className="container">
        ...
        >
          {typedContent.items.map((item, key) => (
            <SwiperSlide className="swiper-slide" key={`course-${key}`}>
                    ...
                    <div className="mil-card-top">
                      <ul className="mil-bages mil-mb-15">
                        {item.tags.map((tag, tagIndex) => (
                          <li className="mil-dark" key={`tag-${key}-${tagIndex}`}>{tag}</li>
                        ))}
                      </ul>
